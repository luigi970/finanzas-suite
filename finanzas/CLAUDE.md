...
- **Gotcha real (corregido 2026-09-03) — "el agente no se conecta" a mitad de charla**: la cuenta free de Groq tiene un límite duro de 8000 tokens/minuto (TPM) para `openai/gpt-oss-120b`. El frontend (`Chat` en App.jsx) manda TODO el historial de la conversación sin límite en cada mensaje nuevo (`[...messages, userMsg]`), sumado al contexto financiero completo (ya pesa varios miles de tokens por sí solo) — a los pocos intercambios de una misma charla la suma pasa el límite y Groq empieza a rechazar el pedido (`413`/`429`), y como en ese momento Gemini (el fallback) también tiene cortes intermitentes de "alta demanda" del lado de Google, el chat parece "dejar de responder" sin razón aparente. Tres causas compuestas, tres fixes:
  1. **Historial sin límite**: `MAX_CHAT_HISTORY = 12` acota cuántos mensajes previos se re-envían al modelo en `/chat` — el historial viejo sigue visible en pantalla, solo deja de mandarse al LLM.
  2. **`max_tokens` faltante inflaba el rechazo por tamaño**: sin `max_tokens` en el body, Groq reserva un presupuesto enorme "por las dudas" para la respuesta y devuelve `413 Request too large` aunque el PROMPT real entre sobrado en el límite (visto en vivo: un prompt de 5,829 tokens reales fue rechazado como "Requested 8899" solo por no declarar `max_tokens`). Fix: siempre pasar `max_tokens` explícito (900 en chat, 1800 en el reporte semanal).
  3. **Razonamiento oculto de `gpt-oss-120b` cuenta contra el mismo límite**: el modelo gasta cientos de tokens de "reasoning" no visibles en la respuesta antes de contestar (visto: 298 de 514 tokens de completion en una respuesta simple). Fix: `"reasoning_effort": "low"` en ambos calls — bajó el reasoning a ~5-80 tokens sin degradar la calidad de la respuesta (el `SYSTEM_PROMPT` ya pide respuestas cortas y directas, no hace falta razonamiento largo).
  - Además, `build_context()` ahora acepta `include_recent_tx: bool` — el reporte semanal (`REPORT_SYSTEM_PROMPT`, casi el doble de largo que el de chat) lo llama con `include_recent_tx=False` para no sumar el detalle de movimientos día a día a un prompt que ya viene muy ajustado al límite; el resumen mensual y el P&L realizado total le alcanzan. El chat sí lo necesita (preguntas tipo "qué hice últimamente") y lo llama con el default `True`.
  - El bloque de movimientos recientes pasó de "últimas 50/20 transacciones" (cantidad fija) a una ventana por FECHA (`WHERE t.date >= date('now', '-10 days')`, tope de seguridad 30) — una cantidad fija podía traer meses viejos en cuentas poco activas, o truncar el mismo día en cuentas muy activas.
  - Helper `_post_gemini()` agrega un reintento corto (2.5s) para el fallback de Gemini — sus 503 "high demand" son casi siempre transitorios.
  - **Al debuggear este tipo de problema**: el `except Exception: pass` alrededor de cada proveedor en `/chat` y `/weekly-report` traga el error real — para ver la causa concreta hay que reproducir la llamada a Groq/Gemini directo en un script aparte (fuera del endpoint), nunca confiar en el genérico "Verificá las API keys" que devuelve el 503 final.
- **Límite estructural, no un bug (encontrado 2026-09-09)**: el 413 de Groq no depende solo del prompt — depende de `prompt_tokens + max_tokens` juntos, tiene que dar ≤ 8000. Medido en vivo con `REPORT_SYSTEM_PROMPT` y la cartera real: prompt ≈7.480 + `max_tokens=1800` (lo que el reporte necesita para salir completo, más ahora que separa CEDEAR de acción real en vez de un solo veredicto) ≈9.280 — **1.280 tokens por encima del techo gratis de Groq, sin importar cuánto se recorte el texto de las reglas**. El reporte semanal cae a Gemini (~40s) la mayoría de las veces en vez del ~5s típico de Groq. Evaluado con el usuario 2026-09-09: se decidió dejarlo así a propósito — es exactamente para esto que existe el fallback, no vale la pena partir el reporte en 2 llamados a Groq ni pasar a un plan pago por una llamada que se usa poco frecuentemente. Si en el futuro se agregan más reglas/contexto al reporte, este límite se puede volver a chocar — no perder tiempo puliendo texto de las reglas para "hacerlo entrar", el margen ya no existe estructuralmente con la cartera actual.
- Fuentes de precios:
