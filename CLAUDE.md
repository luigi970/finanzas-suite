...
- **Fallback Binance → Coinbase → Kraken para datos de cripto (2026-09-11)**: `compute_all()` traía el historial diario de cripto SOLO de Binance (`_fetch_all_binance_daily` en `screener.py`) — Binance geobloquea tráfico que geolocaliza como EE.UU./datacenter de forma intermitente (tienen Binance.US aparte por regulación), y las corridas de GitHub Actions arrancan cada vez en una VM nueva de Azure US, así que a veces pasa y a veces no (confirmado en vivo repetidas veces: un día la lista `crypto` procesó 23/23 pero `watchlist` solo 4/9-4/10; en otro momento hasta la propia lista `crypto` sacó 0/20 — el bloqueo no es exclusivo de la watchlist, es de Binance en general ese momento). Se evaluaron y descartaron CryptoCompare (ahora exige API key hasta en su tier gratis, 401 en vivo) y CoinGecko (solo da velas diarias reales -high/low- para los últimos ~30 días gratis) por degradar la calidad del análisis en silencio. La solución real: `_fetch_all_binance_daily` ahora reintenta en Coinbase y después en Kraken para cualquier ticker que Binance no pudo traer (`_fetch_coinbase_klines`/`_fetch_kraken_klines`) — ambos exchanges dan velas diarias reales (open/high/low/close/volumen, 300+ días) gratis y sin key, PROBADO EN VIVO con los 6 tickers de cripto de la watchlist (100% de éxito, Binance forzado a fallar). A diferencia de Binance, Coinbase y Kraken son empresas de EE.UU. sin motivo para bloquear tráfico de datacenter estadounidense — la razón exacta opuesta al problema original. CoinPaprika se evaluó como tercera opción (mencionado en un artículo de comparativas 2026) y se descartó: su endpoint de histórico devuelve 402 Payment Required en vivo, pese a que la documentación lo muestra como gratis — mismo patrón que CryptoCompare, no confiar en tiers "gratis" de terceros sin probarlos en vivo primero. El botón manual "🔄 Reintentar análisis de hoy" en 🔔 Alertas sigue existiendo como red de seguridad adicional (dispara `POST /api/refresh` con `list_id: "watchlist"`), pero con este fallback debería hacer falta mucho menos. Verificado en producción: la corrida siguiente al deploy pasó de 4/10 a 9/10 tickers procesados (ETH-USD, BTC-USD, BNB-USD, LINK-USD, SOL-USD entraron por Coinbase), y como era la primera vez que esos tickers tenían un registro en `alert_last_signal`, dispararon el aviso de bienvenida — confirmado por el usuario, le llegaron todos con sus noticias. **Guarda de frescura**: `_last_candle_is_fresh()` rechaza (devuelve `None`, la cadena sigue al siguiente proveedor) cualquier vela de Coinbase/Kraken con más de 48hs de antigüedad — sin esto, un bug del lado del proveedor (cache vencido, endpoint devolviendo el día incorrecto) se aceptaría en silencio como si fuera el precio de hoy. Verificado en vivo: la última vela de Binance/Coinbase/Kraken para BTC coincidía con el precio spot del momento (diferencias de centavos, normales entre exchanges distintos, no staleness).
- **Indicador visual de frescura por ticker (2026-09-11)**: si Binance/Coinbase/Kraken fallan los tres para un ticker puntual, su fila en `screener_results` se queda con el análisis de un run anterior en vez de desaparecer — sin nada que lo marque, no había forma de notar que lo que estás mirando no es de hoy. `get_results()` en `worker/src/storage/db.py` ahora suma `updated_at` (ya se guardaba en D1, no se exponía) a cada ticker devuelto por `/api/stocks`. El frontend (`FreshnessBadge` en `App.jsx`) lo muestra en el header del modal de cada ticker y en la sección 🔔 Alertas (ahí como cuándo corrió el job por última vez, no por-ticker — el job puede correr bien y aun así fallarle a algún ticker puntual, como pasó con AAPL en 9/10): gris "🕐 hace Xh" si es reciente (<36hs), ámbar/rojo "⚠️ hace Xd" si no. En desarrollo local no aplica (el modo local siempre recalcula todo en memoria en el momento, no hay dato viejo que arrastrar) — `FreshnessBadge` no renderiza nada si `updated_at` viene `null`.
